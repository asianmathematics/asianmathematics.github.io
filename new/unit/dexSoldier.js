...
DexSoldier.skills = {
    special: [
        ...
        },
        {
            ...
            cost: { stamina: 50 },
            description: "Redirects all non-aoe, non-auto-hit attacks on the frontline to self with increased defense for 1 turn",
            code() {
                ...
                new Modifier("Guardian", "Redirects attacks and increases defense",
                    ...
                    function() {},
                    function(context) {
                        if (context.event === "attackStart" && context.attacker.team !== this.vars.target.team && !(currentAction.at(-2)[0].properties || currentAction.at(-2)[0].vars.properties).some(p => ["aoe", "auto-hit"].includes(p))) {
                            let redirect = 0;
        ...
        },
        {
            ...
            code() { basicModifier("Last Stand", "Defense, resist, and presence increase", { target: this, duration: 5, properties: ["physical", "buff"], stats: { defense: 45, resist: 60, presence: 200 }, listeners: { turnStart: true }, focus: true }); }
        },
        {
            name: "Quake Hammer",
        ...
        },
        {
            ...
            code() {
                ...
                new Modifier("Guardian", "Redirects attacks from an ally and increases defense",
                    ...
                    function() {},
                    function(context) {
                        if (context.event === "attackStart" && context.attacker.team !== this.vars.caster.team && !(currentAction.at(-2)[0].properties || currentAction.at(-2)[0].vars.properties).some(p => ["aoe", "auto-hit"].includes(p))) {
                            let redirect = 0;
    ...
    secondary: [
        ...
        },
        {
            ...
            code() { if (!refreshModifier([{ name: "Last Stand", vars: { caster: this, target: this, parent: this.skills.secondary } }], 2)[0]) basicModifier("Last Stand", "Defense and presence increase", { target: this, duration: 2, properties: ["physical", "buff"], stats: { defense: 20, presence: 75 }, listeners: { turnEnd: true }, focus: true }); }
        }
    ],
    passive: [
        {
        ...
        },
        {
            ...
            code() {
                new Modifier("But It Refused", `Revives`,
                    ...
                    function() {},
                    function(context) { if (context.unit === this.vars.target && context.type === "downed" && resourceChange(this.vars.caster, this.vars.cost, false, false)) { heal(this.vars.caster, [this.vars.target], [1]); } }
                );
            }
        },
        {
            ...
            cost: { stamina: 5 },
            description: "Spends stamina per redirect to redirect all non-aoe, non-auto-hit attacks on ally frontline unit to self",
            code() {
                new Modifier("Guardian", "Redirects attacks",
                    ...
                    function() {},
                    function(context) {
                        if (this.vars.caster.stamina >= this.vars.cost.stamina && !(currentAction.at(-2)[0].properties || currentAction.at(-2)[0].vars.properties).some(p => ["aoe", "auto-hit"].includes(p)) && context.event === "attackStart" && context.attacker.team !== this.vars.target.team) {
                            let redirect = 0;
                            for (let i = 0; i < context.defenders.length; i++) {
                                const target = context.defenders[i];
                                if (target === this.vars.target || target.team !== this.vars.target.team || target.position !== "front" || context.calcMods.defenders?.[i]?.redirect || !resourceChange(this.vars.caster, this.vars.cost, false, false)) continue;
                                context.defenders[i] = this.vars.target;
                                ((context.calcMods.defenders ??= [])[i] ??= {}).redirect = [target, this.vars.target];
        ...
        },
        {
            ...
            code() { basicModifier("Last Stand", "Increases defense and presence", { target: this, properties: ["physical", "buff"], stats: { defense: 15, presence: 50 }, focus: true, passive: true }); }
        },
        {
            name: "Quake Hammer",
    ...
    augment: [
        ...
        },
        {
            ...
            code() { basicModifier("Last Stand", "Increases defense, resist, and presence", { target: this, properties: ["physical", "buff"], stats: { defense: 20, resist: 20, presence: 100 }, focus: true, passive: true }); }
        }
    ]
};
